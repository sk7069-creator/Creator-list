...
function render(){
  ...
  (function(){
    ...
    if(window.__rawFirstRow){
      var cells=window.__rawFirstRow.map(function(v,i){ return i+':'+JSON.stringify(v); }).join(' ');
      h.push('<span style="font-size:10px;color:#888;margin-left:6px;max-width:500px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:inline-block;vertical-align:middle" title="'+esc(cells)+'">'+esc(cells)+'</span>');
    }
  })();
